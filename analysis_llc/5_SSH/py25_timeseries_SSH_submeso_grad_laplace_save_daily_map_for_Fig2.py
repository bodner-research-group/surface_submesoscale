...
out_dir = f"/orcd/data/abodner/002/ysi/surface_submesoscale/Manuscript_Data/{domain_name}/steric_submeso"
...
# Compute gradients and Laplacians for eta
eta = Eta.isel(time=t)
eta_minus_mean = eta - eta.mean(dim=["i", "j"])
eta_grad_mag, eta_laplace = compute_grad_laplace(eta, grid)

# Compute gradients and Laplacians for submesoscale SSH
eta_submeso_t = eta_submeso.isel(time=t)
eta_submeso_grad_mag, eta_submeso_laplace = compute_grad_laplace(eta_submeso_t, grid)

# Compute gradients and Laplacians for steric height
steric_file = os.path.join("/orcd/data/abodner/002/ysi/surface_submesoscale/analysis_llc/data/icelandic_basin/steric_height_anomaly_timeseries", f"grad_laplace_eta_steric_{date_tag}.nc")
eta_steric = xr.open_dataset(steric_file).eta_prime
eta_steric_minus_mean = eta_steric - eta_steric.mean(dim=["i", "j"])
eta_steric_grad_mag, eta_steric_laplace = compute_grad_laplace(eta_steric, grid)
...
ds_day = xr.Dataset(
    {
        ...
        "eta": eta,
        "eta_minus_mean": eta_minus_mean,
        "eta_grad_mag": eta_grad_mag,
        "eta_laplace": eta_laplace,
        "eta_steric": eta_steric,
        "eta_steric_minus_mean": eta_steric_minus_mean,
        "eta_steric_grad_mag": eta_steric_grad_mag,
        "eta_steric_laplace": eta_steric_laplace,        
    },
    coords={
        "lon": lon,
        "lat": lat,
