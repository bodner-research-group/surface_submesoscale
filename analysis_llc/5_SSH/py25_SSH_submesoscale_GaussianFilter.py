...
out_nc_path_submeso = os.path.join(base_out_dir, "SSH_Gaussian_submeso_17.50kmCutoff.nc")
out_nc_path_meso = os.path.join(base_out_dir, "SSH_Gaussian_meso_17.50kmCutoff.nc")
out_nc_path_meso_coarse = os.path.join(base_out_dir, "SSH_Gaussian_meso_17.50kmCutoff_1_12deg.nc")
...
def process_one_timestep(t_index, date_str, eta_day, dx_km):
    """
    Split SSH into submesoscale (<17.50 km) and mesoscale (>17.50 km) components
    ...
    try:
        ...
        # ---- Gaussian filter ----
        sigma_km = 17.50 / np.sqrt(8 * np.log(2))
        ...
        eta_submeso.attrs["description"] = "SSH with > 17.50 km scales removed via Gaussian filter"
        eta_submeso.attrs["filter_cutoff_km"] = 17.50

        eta_meso = eta_large.rename("SSH_mesoscale")
        eta_meso.attrs["description"] = "SSH with < 17.50 km scales removed via Gaussian filter"
        eta_meso.attrs["filter_cutoff_km"] = 17.50
