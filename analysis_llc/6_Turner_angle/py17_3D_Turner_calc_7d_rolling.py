### Compute and plot the 3D Turner Angle for one week, using mean SSH gradient magnitude as z-axis
...
eta_input_dir = f"/orcd/data/abodner/002/ysi/surface_submesoscale/analysis_llc/data/{domain_name}/SSH_7d_rolling"
rho_input_dir = f"/orcd/data/abodner/002/ysi/surface_submesoscale/analysis_llc/data/{domain_name}/rho_Hml_TS_7d_rolling_mean"
out_dir = f"/orcd/data/abodner/002/ysi/surface_submesoscale/analysis_llc/data/{domain_name}/TurnerAngle_3D"
...
# ------------------------------------------
@delayed
def process_tile(j0, j1, i0, i1, tt_np, ss_np, eta_np, dxF_np, dyF_np):
    ...
    deta_dx_tile = np.full((j1 - j0, i1 - i0), np.nan)
    deta_dy_tile = np.full((j1 - j0, i1 - i0), np.nan)
    ...
    for jj, j in enumerate(range(j0, j1)):
        for ii, i in enumerate(range(i0, i1)):
            ...
            eta_win = eta_np[j - 1:j + 2, i - 1:i + 2].flatten()

            # Skip if any missing value
            if np.any(np.isnan(t_win)) or np.any(np.isnan(s_win)) or np.any(np.isnan(eta_win)):
                continue

            # Fit plane to temperature and salinity
            coef_t, *_ = lstsq(A, t_win, rcond=None)
            coef_s, *_ = lstsq(A, s_win, rcond=None)
            coef_eta, *_ = lstsq(A, eta_win, rcond=None)
            ...
            deta_dx_tile[jj, ii] = coef_eta[0] / dxF_np[j, i]
            deta_dy_tile[jj, ii] = coef_eta[1] / dyF_np[j, i]

    return dt_dx_tile, dt_dy_tile, ds_dx_tile, ds_dy_tile, deta_dx_tile, deta_dy_tile
