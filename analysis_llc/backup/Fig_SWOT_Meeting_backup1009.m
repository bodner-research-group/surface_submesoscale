...
plot(ax3, time_qnet, vertical_bf_submeso_mld, 'Color',blue, 'LineWidth', 2);
set(ax3, 'FontSize', fontsize);
ylim([0.5*min(vertical_bf_submeso_mld) 1.2*max(vertical_bf_submeso_mld)])
% ylim([0.5*min(vertical_bf_submeso_mld) 5*max(vertical_bf_submeso_mld)])
% ylim([1e-13 5*max(vertical_bf_submeso_mld)])
...
ylabel(ax3_label, '\boldmath$\mathrm{Submesoscale}\ \overline{wb}^\mathrm{ML}$','Interpreter','latex');
set(ax3_label, 'FontSize', fontsize);
...
l23 = legend(ax3, {'Submesoscale vertical buoyancy flux'},...
    'TextColor', 'k', 'Location', 'northeast','FontSize',fontsize+1,...
    'Position',[0.48 0.8 0.2882 0.0876]);
legend(ax3, 'boxoff');
