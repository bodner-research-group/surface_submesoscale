...
fontsize = 16;
figdir = '~/surface_submesoscale/analysis_llc/figs/icelandic_basin/timeseries/';
...
t_start = dateshift(min(time_Hml), 'start', 'month');
t_end   = dateshift(max(time_Hml), 'start', 'month') + calmonths(1);
...
% figure(10)
% clf;
% plot(time_Hml, mean_spec_in_MLD_submeso, 'LineWidth', 2);
% hold on;
% % plot(time_Hml, mean_spec_in_MLD, 'LineWidth', 2);
...
% Left y-axis: Q_net
yyaxis left
l11 = plot(time_qnet, -qnet_7day_smooth, 'Color',blue, 'LineWidth', 2);
ylabel('\boldmath$\mathcal{Q}_\mathrm{net}\ (\mathrm{W/m^2})$','Interpreter','latex');
ax1 = gca;
ax1.YColor = blue;  % Blue axis
...
xlim([min(time_Hml), datetime(2012,11,1)]);
...
% Legends
l1112 = legend(ax1, [l11 l12] ,{'Net surface heat flux', 'Mixed layer base'}, ...
    'Location', 'northwest','FontSize',fontsize+2,'Position',[0.1098+0.04 0.7339+0.025 0.2518 0.1632]);
l13 = legend(ax3, {'Mixed layer stratification'},...
    'TextColor', green, 'Location', 'northeast','FontSize',fontsize+2,...
...
plot(ax3, time_Hml, mean_spec_in_MLD_submeso, 'Color',blue, 'LineWidth', 2);
ylabel(ax3, '\boldmath$wb$','Interpreter','latex');
set(ax3, 'FontSize', fontsize);
ylim([0.5*min(mean_spec_in_MLD_submeso) 5*max(mean_spec_in_MLD_submeso)])
% ylim([1e-13 5*max(mean_spec_in_MLD_submeso)])
...
ylabel(ax3_label, '\boldmath$wb$','Interpreter','latex');
set(ax3_label, 'FontSize', fontsize);
...
leg2122 = legend([l21 l22],{'Turner Angle Agreement', 'SSH gradient magnitude'},...
    'Location', 'northwest','FontSize',fontsize+2,'Position', [0.1309-0.03 0.7425+0.02 0.3128 0.1632]);
l23 = legend(ax3, {'wb'},...
    'TextColor', blue, 'Location', 'northeast','FontSize',fontsize+2,...
    'Position',[0.5273-0.01 0.8095+0.025 0.2882 0.0876]);
...
% 
% yyaxis left
% % l21 = plot(time_Hml, squeeze(Tu_agreements_pct), 'LineWidth', 2);
