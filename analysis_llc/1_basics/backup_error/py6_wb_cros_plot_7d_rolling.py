...
for t_idx, time_val in enumerate(times):
    ...
    # ax.text(kr_cutoff_meso, ax.get_ylim()[0]*1.1, '30 km cutoff',
    #         color='r', ha='right', va='middle', rotation=90)
    # ax.legend()
    # plt.tight_layout()
...
max_Lr_np = np.array(max_Lr)
max_depths_np = np.array(max_depths)
...
axs[0].plot(times_np[valid_mask], -max_values_np[valid_mask], marker='o', color='tab:blue')
...
# (b) Wavelength
axs[1].plot(times_np[valid_mask], max_Lr_np[valid_mask], marker='o', color='tab:orange')
axs[1].set_title("(b) Horizontal Length Scale (Lmax)")
axs[1].set_ylabel("Lmax (km)")
axs[1].grid(True)

# (c) Depth at min
axs[2].plot(times_np[valid_mask], max_depths_np[valid_mask], marker='o', color='tab:green')
