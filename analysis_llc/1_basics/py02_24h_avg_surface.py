##### Compute 24-hour averages of surface U, V, and SSH. Save as .nc files.
...
def compute_and_save_weekly(var_name, label, i_name, j_name):
    ...
    for n in range(total_segments):
        ...
        # Select data slice
        da = ds1[var_name].isel(time=slice(t0, t1), face=face, **{i_name: i, j_name: j})

        # If 'k' is in dimensions, select surface layer
        if 'k' in ds1[var_name].dims:
            da = da.isel(k=0)
...
############## Another option: use Dask delayed
#
