...
lgd2 = legend(ax1b,[p2r p3r], ...
    {'\textbf{Surface buoyancy flux $\rho_0\langle B_\mathrm{surf}\rangle/(g\Delta\rho)$}',...
    '\textbf{Ekman buoyancy flux $\rho_0 \langle B_\mathrm{Ek}\rangle/(g\Delta\rho)$}'}, ...
    ...% 'NumColumns', 2,...
    'FontSize',fontsize-1,'Interpreter','latex','Position', [0.3962 0.8839 0.5112 0.075]);
lgd2.Box = 'off';
...
lgd4 = legend(ax1d,[p7r], ...
    {'\textbf{Diagnosed VBF $\langle-\overline{w^\prime b^\prime}^{xyz}\rangle$}'}, ...
    'NumColumns', 2,...
    'FontSize',fontsize-1,'Interpreter','latex','Position',   [-0.166 0.5774 0.8545 0.0626]);
lgd4.Box = 'off';
...
ax2 = axes('Position',[0.09 0.05 0.85 0.36]);
hold(ax2,'on')
...
h2 = plot(tw_steric, Hml_recon_steric_2012, 'Color',brown, 'LineWidth', 2.5);
h3 = plot(tw_steric, Hml_recon_submeso_2012, 'Color',olive, 'LineWidth', 2.5);
...
hvl = plot(ax2, [x_event x_event], [425 725], ':','Color',red, 'LineWidth', 2);
...
legend([h1 h2 h3],{'\textbf{Model-diagnosed MLD $\langle\it{H}\rangle$}',...
    ...
    'FontSize', fontsize,'Interpreter','latex','Position',[0.2021 0.0788 0.4716 0.12])
legend('boxoff')
