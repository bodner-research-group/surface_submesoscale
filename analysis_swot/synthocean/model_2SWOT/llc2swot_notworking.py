...
def read_netcdf_files(file1, file2, file3, time_name = "time_counter", time_step=0):
    ...
    ds1 = xr.open_dataset(file1) if extension1 == ".nc" else xr.open_zarr(file1, consolidated=False) # model
...
def open_model_data(ds_var, ds_coords, interpolator, var, latitude_array, longitude_array, 
                    lat_name="latitude", lon_name="longitude", time_step=0):
    """
    Prepare interpolator from model data (supporting both regular and LLC4320 grids).
    
    Parameters:
    - ds_var: xarray.Dataset containing the variable (e.g., Eta)
    - ds_coords: Dataset containing lat/lon fields (or same as ds_var)
    - interpolator: Interpolator method string: 'scipy_interpolator' or 'pyinterp_interpolator'
    - var: Variable name to interpolate (e.g., Eta or ssh)
    - latitude_array: SWOT latitude array
    - longitude_array: SWOT longitude array
    - lat_name, lon_name: Names of lat/lon variables (ignored for LLC4320)
    - time_step: Time index to use
    
    Returns:
    - finterp: Interpolator object, or 0 if empty
    """
    import numpy as np
    from scipy import interpolate
    import pyinterp
    ...
    # --- Handle LLC4320: face-based grid (face, j, i) ---
    is_llc = False
    if var in ds_var:
        var_dims = ds_var[var].dims
        if "face" in var_dims:
            is_llc = True
    ...
    if is_llc:
        ...
        print("Detected LLC4320 format. Using llcreader to convert face data to lat/lon grid...")

        # Determine the root path from ds_var encoding
        if 'source' in ds_var.encoding:
            llc_path = ds_var.encoding['source']
        else:
            raise ValueError("LLC4320 dataset encoding must contain 'source' path.")

        # Open model
        model = llcreader.LLC4320Model(store=llc_path)

        # Read full lat/lon interpolated view
        ds_llc = model.get_dataset(varnames=[var], type='latlon', k_levels=None)

        # Get time slice
        var_values = ds_llc[var].isel(time=time_step).values
        lat_values = ds_llc['lat'].values
        lon_values = ds_llc['lon'].values
        ...
        try:
            lat_values = ds_coords[lat_name].values
            lon_values = ds_coords[lon_name].values
        except KeyError:
            raise ValueError(f"Latitude or longitude variables '{lat_name}'/'{lon_name}' not found in ds_coords.")

        var_values = ds_var[var].values

        # If 3D (time, y, x), select timestep
        if var_values.ndim == 3:
            var_values = var_values[time_step]

    # Select subset inside SWOT swath
    lon_in, lat_in, var_in = select_area(
        lon=lon_values,
        lat=lat_values,
        var=var_values,
...
def main():
    ...
    parser.add_argument("--model_timestep_index", type=int, default=0, 
    ...
    # Pre-check existence of input paths (file or directory)
    for f in [args.model_file, args.mask_file, args.swot_file]:
        if not os.path.exists(f):
            parser.error(f"Error: Input path does not exist: {f}")
        if not (os.path.isfile(f) or os.path.isdir(f)):
            parser.error(f"Error: Path is neither a file nor a directory: {f}")
